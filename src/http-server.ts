	...
	*/
	private setupRoutes(): void {
		...
		this.app.get('/describe', (_req, res) => {
			...
			const describeDoc = createDescribe({
				metadata: {
					name:        'ghl-mcp-server',
					version:     process.env.npm_package_version ?? '1.0.0',
					description: 'HTTP endpoint for ChatGPT MCP integration',
				},
				tools: allTools,
			});
			
			// 3) Return it
			res.json(describeDoc);
		});
		...
		this.app.get('/mcp', async (req: express.Request, res: express.Response): Promise<void> => {
			...
			// Implementation depends on your specific SSE needs
